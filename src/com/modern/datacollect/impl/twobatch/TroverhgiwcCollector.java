...
public class TroverhgiwcCollector extends TroversCollector {
	...
	public void begin() {
		...
		if (config == null) {// 开发时用到的，自己配置
			...
			config.setSiteUrl("http://www.trover.com/l/giwc-surf-spots");
			// 更新配置每次抓取一页数据,可用用于配置，当前抓取第几页，第几条数据。
			config.setSiteConfig("{'page':0,'dataUrl':'http://www.trover.com/discoveries/wall?list=giwc&ll=0.0,0.0&name=Seoul&anonymous=0&limit=24&offset={page}&callback=$'}");
			// 文件的保存正式目录
		...
		for (;;) {
			try {
				...
				config.setSiteConfig("{'page':" + page + ",'dataUrl':'dataUrl':'http://www.trover.com/discoveries/wall?list=giwc&ll=0.0,0.0&name=Seoul&anonymous=0&limit=24&offset={page}&callback=$'}");
