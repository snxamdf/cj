...
public class HdbChengduCollector extends HdbsCollector {
	...
	public void begin() {
		...
		if (config == null) {// 开发时用到的，自己配置
			...
			config.setSiteConfig("{'page':1,'dataUrl':'chengdu-sjbx-p{page}'}");
			// 文件的保存正式目录
		...
		for (;;) {
			try {
				...
				config.setSiteConfig("{'page':" + page + ",'dataUrl':'chengdu-sjbx-p{page}'}");
