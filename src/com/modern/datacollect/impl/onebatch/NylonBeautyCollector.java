...
public class NylonBeautyCollector extends Collector {
	...
	public void begin() {
		...
		if (config == null) {// 开发时用到的，自己配置
			...
			config.setSiteUrl("http://www.nylon.com/beauty");
			// 更新配置每次抓取一页数据,可用用于配置，当前抓取第几页，第几条数据。
			config.setSiteConfig("{'page':1,'dataUrl':'https://api.nylon.com/api/v2/page/beauty/page-{page}'}");
			// 文件的保存正式目录
		...
		for (;;) {
			try {
				...
				config.setSiteConfig("{'page':" + (page) + ",'dataUrl':'https://api.nylon.com/api/v2/page/beauty/page-{page}'}");
