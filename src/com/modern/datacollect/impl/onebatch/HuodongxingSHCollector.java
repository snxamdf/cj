...
public class HuodongxingSHCollector extends HuodongxingsCollector {
	...
	public void begin() {
		...
		if (config == null) {// 开发时用到的，自己配置
			...
			config.setSiteConfig("{'page':1,'dataUrl':'?orderby=r&city=上海&page={page}'}");
			// 文件的保存正式目录
		...
		for (;;) {
			try {
				...
				config.setSiteConfig("{'page':" + page + ",'dataUrl':'?orderby=r&city=上海&page={page}'}");
