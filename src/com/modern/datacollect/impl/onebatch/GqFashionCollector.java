...
public class GqFashionCollector extends Collector {
	...
	public void begin() {
		...
		if (config == null) {// 开发时用到的，自己配置
			...
			config.setSiteUrl("http://www.gq.com.cn/fashion/#pid=3");
			// 更新配置每次抓取一页数据,可用用于配置，当前抓取第几页，第几条数据。
			config.setSiteConfig("{'page':0,'dataUrl':'http://www.gq.com.cn/front_ajax_channel_v1/readchannelmore/2/?pg={page}'}");
			// 文件的保存正式目录
		...
		for (;;) {
			try {
				if (page == 0) {
					...
					Elements body = Tools.getBody("div[node-id=\"container\"]", html);
					this.dealwith(body.select(".articleboxmin"), tempFileDir, targetFileDir);
					page++;
				}
				url = dataUrl.replace("{page}", page.toString());

				config.setSiteConfig("{'page':" + (page) + ",'dataUrl':'http://www.gq.com.cn/front_ajax_channel_v1/readchannelmore/2/?pg={page}'}");
