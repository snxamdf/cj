...
public class LotourChujingyouCollector extends Collector {
	...
	public void begin() {
		...
		if (config == null) {// 开发时用到的，自己配置
			...
			config.setSiteUrl("http://www.lotour.com/chujingyou/");
			// 更新配置每次抓取一页数据,可用用于配置，当前抓取第几页，第几条数据。
			config.setSiteConfig("{'page':1,'dataUrl':'http://api.lotour.net/brandhome/water/SelectDiscoverList?regionId=0&pageSize=50&pageIndex={page}&isbroad=1'}");
			// 文件的保存正式目录
